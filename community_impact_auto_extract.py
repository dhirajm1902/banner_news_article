#!/usr/bin/env python3
"""
Community Impact Auto Extractor
Reads community_impact_latest.json → fetches each article → calls Groq API
(free) with structured extraction prompt → saves to
community_impact_extraction_latest.json and
master_file/community_impact_master_extraction.csv.

Usage:
    python community_impact_auto_extract.py
    python community_impact_auto_extract.py --batch-size 5
    python community_impact_auto_extract.py --max-articles 10
    python community_impact_auto_extract.py --reset
...
# ── Config ─────────────────────────────────────────────────────────────────────
OUT_PATH    = Path("community_impact_extraction_latest.json")
MASTER_DIR  = Path("master_file")
MASTER_FILE = MASTER_DIR / "community_impact_master_extraction.csv"
BATCH_SIZE  = 5
...
SYSTEM_PROMPT = """\
You are an expert, precise data extractor specialized in retail and restaurant openings and closures. I will provide multiple local-news articles from Community Impact (each starting with its source URL and metro city). For EVERY article, extract the following information strictly and only from the text provided — no assumptions, no external knowledge, no guessing zip codes, no inferring dates or statuses:

🔍 Extract these fields
• Store/Shop/Restaurant Name
• Location or Full Address with zip code (if no zip code is mentioned, write exactly the address given; if no address at all, write the metro city given in the article metadata)
...
def main():
    ...
    ci_path = Path("community_impact_latest.json")
    if not ci_path.exists():
        print("❌  community_impact_latest.json not found.")
        sys.exit(1)
    ...
    if not articles:
        print("No articles in community_impact_latest.json — nothing to extract.")
        return
